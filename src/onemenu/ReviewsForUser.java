...
@WebServlet("/YourReviews")
public class ReviewsForUser extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    public ReviewsForUser() {
        super();
    	...
    	if(request.getSession().getAttribute("username") == null || request.getSession().getAttribute("userID") == null ){
			...
			try {
				...
				String sql = "select m.name as foodName, m.restaurant, rest.name as restName, u.firstName, u.lastName, r.* from Reviews r join Users u on r.userID = u.userID join Menu m on r.foodID = m.id  join Restaurants rest on rest.restID = m.restaurant "
						+ "	  WHERE u.userID=" + "'" + request.getSession().getAttribute("userID") + "' order by(reviewDate) desc;";
				...
				if(!rs.isBeforeFirst()){
					request.setAttribute("NoReviewsExist", true);
					request.getRequestDispatcher("/YourReviews.jsp").forward(request, response);
					return ;
				}
				while (rs.next()) {
					System.out.println("herehehehehe");
					Review r = new Review() ;
					
					r.setFoodName(rs.getString("foodName")); 
					r.setRestaurantName(rs.getString("restName")); 
				...
				String sql2 = "select COUNT(*) as totalReviews, AVG(rating) as averageRating from Reviews WHERE userID=" + "'" +request.getSession().getAttribute("userID") + "'";
